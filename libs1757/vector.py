...
class Point(object):
    ...
    def subtract(self, pt):
        """Return a Point instance as the relative coordinates of 'pt' with respect to 'self'."""
        if isinstance(pt, Point):
                return Point(pt.x - self.x, pt.y - self.y, pt.z - self.z)
        else:
            raise TypeError
...
class Vector(Point):
    ...
    def non_parallel(self, vector):
        ...
        """
        if (self.isParallel(vector) is not True and
                self.isPerpendicular(vector) is not True):
            return True
        return False
    ...
    def from_points(cls, point1, point2):
        ...
        if isinstance(point1, Point) and isinstance(point2, Point):
            displacement = point1.subtract(point2)
...
# bv = Vector(0.5, 0., 0.5)
# avmbv = av - bv
#ap2bp = Vector.from_points(ap, bp)
# print(av)
# print(bv)
# print(avmbv)
# print(av.magnitude())
# print(bv.magnitude())
# print(avmbv.magnitude())
#print(ap2bp)
